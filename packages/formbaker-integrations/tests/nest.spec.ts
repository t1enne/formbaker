...
describe("nest class-validator code generation", () => {
  ...
  it.each([
    ...
    [
      ...
      ["@IsNumber()", "@Min(18)", "@Max(120)", "@IsDefined()"],
      "number",
    ],
  ...
  it("omits imports when includeImports is false", () => {
    ...
    expect(
      formbakerToClassValidator(form, { className: "Dto", includeImports: false }),
  ...
  it("applies class-level decorators", () => {
    ...
    expect(
      formbakerToClassValidator(form, { className: "Dto", classDecorators: ["@ApiProperty()"] }),
    ).toContain("@ApiProperty()");
  });

  it("generates empty class when no fields", () => {
    const code = formbakerToClassValidator(create({ pluginName: "zod" }), {
      className: "EmptyDto",
    });
